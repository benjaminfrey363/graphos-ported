...
#include <stdexcept>
//#include "sgx_trts.h"
#include "include/HeapObliviousOperations.h"
...
HeapBucket DOHEAP::DeserialiseBucket(block buffer) {
    ...
    for (int z = 0; z < Z; z++) {
        ...
        for (int k = 0; k < node->value.size(); k++) {
            ...
            node->value[k] = HeapNode::conditional_select(node->value[k], static_cast<byte_t>(0), static_cast<int>(!cond));
        }
        for (int k = 0; k < node->key.id.size(); k++) {
            node->key.id[k] = HeapNode::conditional_select(node->key.id[k], static_cast<byte_t>(0), static_cast<int>(!cond));
        }
