...
#include <stdexcept>
//#include "sgx_trts.h"
#include "include/ObliviousOperations.h"
...
Node* ORAM::ReadWrite(Bid bid, Node* inputnode, unsigned long long lastLeaf, unsigned long long newLeaf, bool isRead, bool isDummy, bool isIncRead) {
    //printf("[DEBUG] ORAM::ReadWrite(Node*, isRead=%d, isIncRead=%d) CALLED\n", isRead, isIncRead); // <-- ADD THIS
    if (bid == 0) {
...
Node* ORAM::ReadWrite(Bid bid, unsigned long long lastLeaf, unsigned long long newLeaf, bool isDummy, unsigned long long newChildPos, Bid targetNode) {
    //printf("[DEBUG] ORAM::ReadWrite(..., targetNode) CALLED\n"); // <-- ADD THIS
    if (bid == 0) {
        printf("bid is 0 dummy is:%d\n", isDummy ? 1 : 0);
        throw runtime_error("Node id is not set");
    }
...
Node* ORAM::ReadWrite(Bid bid, unsigned long long lastLeaf, unsigned long long newLeaf, bool isDummy, unsigned long long newChildPos, Bid targetNode, string part, bool isFirstPart) {
    //printf("[DEBUG] ORAM::ReadWrite(..., isFirstPart=%d) CALLED\n", isFirstPart); // <-- ADD THIS
    if (bid == 0) {
        printf("bid is 0 dummy is:%d\n", isDummy ? 1 : 0);
        throw runtime_error("Node id is not set");
    }
    ...
    for (Node* node : stash.nodes) {
        ...
        for (int i = 0; i < node->value.size(); i++) {
            tmpRes[i] = Node::conditional_select(node->value[i], static_cast<byte_t>(tmpRes[i]), static_cast<int>(curNodeIsTarget));
        }

        /*
        // ------------------- ADD THIS SPY CODE BLOCK -------------------
        string currentKeyAsString;
        currentKeyAsString.assign(node->key.id.begin(), node->key.id.end());
        // Trim null characters for clean printing
        currentKeyAsString.erase(std::find(currentKeyAsString.begin(), currentKeyAsString.end(), '\0'), currentKeyAsString.end());

        if (currentKeyAsString == "?1") {
            printf("\n--- ORAM SPY --- \n");
            printf("Key Match: %s\n", currentKeyAsString.c_str());
            printf("Value Read: %s\n", tmpRes.c_str());
            printf("isFirstPart flag: %d\n", isFirstPart);
            printf("----------------\n\n");
        }
        // ---------------------------------------------------------------
        */
