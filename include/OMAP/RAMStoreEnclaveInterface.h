...
// made inline to avoid multiple definitions
inline RAMStore* runStore = NULL;
inline RAMStore* setupStore = NULL;
inline RAMStore* heapStore = NULL;

inline bool setupMode = false;

inline void ocall_setup_heapStore(size_t num, int size) {
    if (heapStore == NULL) {
            heapStore = new RAMStore(num, false);
    }
}

inline void ocall_setup_ramStore(size_t num, int size) {
    if (setupMode) {
        if (setupStore == NULL) {
            if (size != -1) {
...
inline void ocall_nwrite_ramStore(size_t blockCount, long long* indexes, const char *blk, size_t len) {
...
inline void ocall_nwrite_heapStore(size_t blockCount, long long* indexes, const char *blk, size_t len) {
    assert(len % blockCount == 0);
    size_t eachSize = len / blockCount;
    for (unsigned int i = 0; i < blockCount; i++) {
        block ciphertext(blk + (i * eachSize), blk + (i + 1) * eachSize);
...
inline void ocall_write_rawRamStore(long long index, const char *blk, size_t len) {
    size_t eachSize = len;
...
inline void ocall_write_prfRamStore(long long index, const char *blk, size_t len) {
    size_t eachSize = len;
    block ciphertext(blk, blk + eachSize);
    if (setupMode) {
...
inline void ocall_nwrite_rawRamStore(size_t blockCount, long long* indexes, const char *blk, size_t len) {
    assert(len % blockCount == 0);
    size_t eachSize = len / blockCount;
    for (unsigned int i = 0; i < blockCount; i++) {
        block ciphertext(blk + (i * eachSize), blk + (i + 1) * eachSize);
...
inline void ocall_nwrite_ramStore_by_client(vector<long long>* indexes, vector<block>* ciphertexts) {
...
inline void ocall_nwrite_raw_ramStore(vector<block>* ciphertexts) {
...
inline void ocall_finish_setup() {
    setupMode = false;
}

inline void ocall_begin_setup() {
    setupMode = true;
}

inline void ocall_nwrite_rawRamStore_for_graph(size_t blockCount, long long* indexes, const char *blk, size_t len) {
    assert(len % blockCount == 0);
    size_t eachSize = len / blockCount;
    for (unsigned int i = 0; i < blockCount; i++) {
        block ciphertext(blk + (i * eachSize), blk + (i + 1) * eachSize);
...
inline size_t ocall_nread_ramStore(size_t blockCount, long long* indexes, char *blk, size_t len) {
    assert(len % blockCount == 0);
    size_t resLen = -1;
    for (unsigned int i = 0; i < blockCount; i++) {
...
inline size_t ocall_nread_heapStore(size_t blockCount, long long* indexes, char *blk, size_t len) {
    assert(len % blockCount == 0);
    size_t resLen = -1;
    for (unsigned int i = 0; i < blockCount; i++) {
...
inline size_t ocall_read_rawRamStore(size_t index, char *blk, size_t len) {
    size_t resLen = -1;
...
inline size_t ocall_read_prfRamStore(size_t index, char *blk, size_t len) {
    size_t resLen = -1;
...
inline size_t ocall_nread_rawRamStore(size_t blockCount, size_t begin, char *blk, size_t len) {
    assert(len % blockCount == 0);
    size_t resLen = -1;
...
inline size_t ocall_nread_prf(size_t blockCount, size_t begin, char *blk, size_t len) {
    assert(len % blockCount == 0);
    size_t resLen = -1;
...
inline void ocall_initialize_ramStore(long long begin, long long end, const char *blk, size_t len) {
...
inline void ocall_initialize_heapStore(long long begin, long long end, const char *blk, size_t len) {
    block ciphertext(blk, blk + len);
    for (long long i = begin; i < end; i++) {
...
inline void ocall_write_ramStore(long long index, const char *blk, size_t len) {
    block ciphertext(blk, blk + len);
    if (setupMode) {
...
inline void ocall_write_heapStore(long long index, const char *blk, size_t len) {
    block ciphertext(blk, blk + len);
    heapStore->Write(index, ciphertext);
}

inline void ocall_nwrite_prf(size_t blockCount, long long* indexes, const char *blk, size_t len) {
    assert(len % blockCount == 0);
    size_t eachSize = len / blockCount;
    for (unsigned int i = 0; i < blockCount; i++) {
        block ciphertext(blk + (i * eachSize), blk + (i + 1) * eachSize);
